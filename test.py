...
def standardization(input_data):
    length = np.size(input_data, 0)
    data_mean = np.mean(input_data, axis=1).reshape(length, 1)
    data_sigma = np.std(input_data, axis=1).reshape(length, 1)
...
def attention(query, value_list):
    ...
    # 求加权和
    return standardization(np.dot(similarities, value_list))
